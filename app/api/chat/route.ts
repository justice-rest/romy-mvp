...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { message, chatId, trigger, messageId, isNewChat, promptEnhancement } = body
    ...
    if (!userId) {
      // Guest user - check guest rate limit
      const guestLimitResponse = await checkAndEnforceGuestLimit(req)
    ...
    // Check rate limit for quality mode (authenticated users only)
    ...
    const response = await createChatStreamResponse({
      ...
      model: selectedModel,
      chatId,
      userId: userId ?? null, // userId may be null for guest users
      trigger,
      messageId,
      abortSignal,
      isNewChat,
    ...
    // Track analytics event (non-blocking) - only for authenticated users
    if (userId) {
      ;(async () => {
        try {
