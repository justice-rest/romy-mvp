...
export async function prepareMessages(
  ...
  if (trigger === 'regenerate-message' && messageId) {
    // Guest users cannot regenerate messages (no chat history)
    if (!userId) {
      throw new Error('Authentication required for message regeneration')
    }
    ...
    if (isNewChat) {
      // Guest users: return message without persisting
      if (!userId) {
        perfLog('Guest user - skipping database persistence')
        perfTime('prepareMessages - Return (guest, no persistence)', startTime)
        return [messageWithId]
      }

      // Authenticated users: persist the chat and first message optimistically in the background
    ...
    // Guest users: return message without persisting
    if (!userId) {
      perfLog('Guest user - skipping database operations for existing chat')
      if (initialChat && initialChat.messages) {
        perfTime('prepareMessages - Total (guest, cached)', startTime)
        return [...initialChat.messages, messageWithId]
      }
      perfTime('prepareMessages - Total (guest, no cache)', startTime)
      return [messageWithId]
    }
