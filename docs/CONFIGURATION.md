...
By default, Rōmy runs in **anonymous mode** with authentication disabled (`ENABLE_AUTH=false` in `.env.local.example`). This is ideal for personal use where all users share a single anonymous user ID.
...
With authentication enabled, users will need to sign up/login to use Rōmy, and each user will have isolated chat history.
...
Define all four combinations to control which model runs for every search mode (`quick`, `adaptive`) and preference (`speed`, `quality`). For example, you can pair `quick/speed` with `sonar` while keeping `adaptive/quality` on `sonar-pro`. The default config ships with Perplexity Sonar models for every slot so Rōmy works out-of-the-box.
...
[Perplexity](https://www.perplexity.ai/) provides Sonar models with real-time web search capabilities and grounded responses.
...
- `sonar` - 128K context window, optimized for speed
...
- **Tools Capability**: Rōmy requires models to support the `tools` capability for function calling. On server startup, Rōmy validates configured models and logs the results. Note that even if a model reports tools support, actual tool calling performance depends on the model's capabilities and is not guaranteed.
